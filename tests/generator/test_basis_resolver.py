...
from atomref_proatoms.generator.basis_resolver import (
    ...
    detect_ecp_symbols_in_nwchem_text,
    parse_basis_spec,
)
...
def test_file_basis_text_scan_detects_coverage_and_ecp(tmp_path: Path) -> None:
    ...
    assert detect_ecp_symbols_in_nwchem_text(ecp_text) == ("C",)
...
def test_apply_basis_policy_rejects_wfn_with_ecp_even_with_override(tmp_path: Path) -> None:
    path = tmp_path / "ecp.nw"
    path.write_text(
        'BASIS "ao basis" SPHERICAL PRINT\n'
        "C S\n"
    ...
    result = check_basis_source(parse_basis_spec(basis_file=path), ("C",))
    decision = apply_basis_policy(
        result,
        BasisPolicy(allow_ecp=True, artifact_requires_wfn=True),
    )

    assert result.ecp_symbols == ("C",)
    assert decision.status == "error"
    assert any("WFN export requires all-electron" in error for error in decision.errors)
