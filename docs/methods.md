...
The `.rad` exporter evaluates each local spherical SCF density directly on the fixed Multiwfn `atmrad` radial grid and writes one file per atom/ion using the conventional charge suffix, for example `H_0.rad`, `O-1.rad`, or `Fe+2.rad`. The default evaluation samples one fixed Cartesian ray of the one-center spherical SCF density; an angular-average mode remains available for local diagnostics. The exporter deliberately does not derive `.rad` files by interpolating the committed wide profile tables. It records finite-grid electron integrals and source SCF paths as validation metadata. These checks are format-boundary diagnostics; they do not implement a Hirshfeld-I cycle.
